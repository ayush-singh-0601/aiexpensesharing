...
export const getExpensesBetweenUsers = query({
    ...
    handler: async(ctx, {userId})=>{
        ...
        if(me._id === userId.id) throw new Error("Cannot query yourself");
        ...
        const thierPaid= await ctx.db.query("expenses").withIndex("by_user_and_group", (q)=>
        q.eq("paidByUserId", userId).eq("groupId", undefined)
        ).collect();
        
        const candidateExpenses = [...myPaid, ...thierPaid];
...
export const deleteExpense= mutation({
    ...
    handler: async (ctx, args)=>{
        ...
        if(expense.createdBy !== user._id && expense.paidByUserId !== user._id){
            throw new Error ("You don't have permission to delete the expense");
        }
